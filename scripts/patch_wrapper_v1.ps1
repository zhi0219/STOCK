...
function Fail {
  ...
  # PATCHBOT_FAIL_V3_START
  if ([string]::IsNullOrWhiteSpace($reason)) {
    if ($rest -and $rest.Count -gt 0) { $reason = [string]$rest[0] }
  }
  if ([string]::IsNullOrWhiteSpace($reason)) { $reason = "unknown_failure" }
  $reason = ($reason -replace "`r?`n"," ").Trim()
  Write-Host ("WRAP_SUMMARY|status=FAIL|reason=" + $reason)
  exit 1
  # PATCHBOT_FAIL_V3_END
}
...
Set-Location $repo
# PATCHBOT_BLOCK_APPLY_ON_MAIN_START
if ($Confirm -eq "APPLY") {
  $cur = (GitOK rev-parse --abbrev-ref HEAD).Trim()
  if ($cur -eq "main") { Fail "refuse_apply_on_main|next=git checkout -b <branch>" }
}
# PATCHBOT_BLOCK_APPLY_ON_MAIN_END
