...
def _py_compile_targets() -> List[Path]:
    ...
    for name in [
        ...
        "verify_powershell_null_safe_trim_contract.py",
...
def check_powershell_null_safe_trim_contract(
    artifacts_dir: Path, python_exec: str
) -> List[CheckResult]:
    cmd = [
        python_exec,
        "-m",
        "tools.verify_powershell_null_safe_trim_contract",
        "--artifacts-dir",
        str(artifacts_dir),
    ]
    try:
        ...
        return [CheckResult("powershell null-safe trim", False, f"error={exc}")]
    if completed.returncode == 0:
        return [CheckResult("powershell null-safe trim", True)]
    detail = (
        "exit_code="
        f"{completed.returncode}; see {artifacts_dir / 'verify_powershell_null_safe_trim_contract.txt'}"
    )
    return [CheckResult("powershell null-safe trim", False, detail)]
...
def check_inventory_docs_posix_paths(artifacts_dir: Path, python_exec: str) -> List[CheckResult]:
    cmd = [
        python_exec,
        "-m",
    ...
    try:
        ...
        return [CheckResult("inventory docs posix paths", False, f"error={exc}")]
    if completed.returncode == 0:
        return [CheckResult("inventory docs posix paths", True)]
    detail = (
        "exit_code="
        f"{completed.returncode}; see {artifacts_dir / 'verify_inventory_docs_posix_paths.txt'}"
    )
    return [CheckResult("inventory docs posix paths", False, detail)]
...
def main(argv: List[str] | None = None) -> int:
    ...
    p0_checks: List[Callable[[], List[CheckResult]]] = [
        ...
        lambda: check_powershell_null_safe_trim_contract(
            Path(args.artifacts_dir), python_exec
        ),
        ...
        lambda: check_inventory_docs_posix_paths(Path(args.artifacts_dir), python_exec),
