...
def main() -> int:
    ...
    if ui_scroll.returncode != 0 or ui_scroll_status == "FAIL":
        ...
        degraded = True
        degraded_reasons.append(ui_scroll_summary.get("reason", "ui_display_unavailable"))

    ui_smoke = _run([sys.executable, str(TOOLS_DIR / "verify_ui_smoke.py")])
    if ui_smoke.stdout:
        print(ui_smoke.stdout.strip())
    if ui_smoke.stderr:
        print(ui_smoke.stderr.strip())
    ui_smoke_summary = _extract_summary(ui_smoke.stdout, "UI_SMOKE_SUMMARY")
    ui_smoke_status = ui_smoke_summary.get("status")
    if ui_smoke.returncode != 0 or ui_smoke_status == "FAIL":
        status = "FAIL"
        reasons.append("ui_smoke_failed")
    elif ui_smoke_status == "SKIP":
        degraded = True
        degraded_reasons.append(ui_smoke_summary.get("reason", "ui_display_unavailable"))
    elif not ui_smoke_summary:
        status = "FAIL"
        reasons.append("ui_smoke_missing_summary")
